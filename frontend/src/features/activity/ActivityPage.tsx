        ...
        <div className="overlay-panel p-5">
          ...
          <div className="space-y-2">
            {browserTabs.slice(0, 8).map((tab) => {
              const label = tab.display_name || getBrowserTabDisplayName(tab.window_title, tab.url);
              return (
              <div key={`${label}-${tab.url || tab.window_title}`} className="flex items-center justify-between gap-4 py-2 border-b border-white/5 last:border-0">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-white truncate" title={tab.window_title}>{label}</p>
                </div>
